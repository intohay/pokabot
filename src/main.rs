...
async fn tweet_blog(post_id: i32 ,twitter: &Twitter, chatgpt: &ChatGPT, scraper: &Scraper, lang: &str, connection: &mut SqliteConnection) -> anyhow::Result<()>{
    ...
    let prompt = if lang == "jp" {  
        if name == "ポカ" {
            "---\n以下のブログを書いた本人になりきって、日本語50字以内で短めに、ブログの宣伝ツイートをしてください。ただし、最終的な文字数が50字を超えていた場合は、適宜不必要な部分を削って50字以内に収まるようにしてください。"
        } else {
            "---\n以下のアイドルのブログを読んだ感想を、彼女のファンになったつもりで、カジュアルな口調で、日本語50字以内で短めにツイートしなさい。ただし、最終的な文字数が50字を超えていた場合は、適宜不必要な部分を削って50字以内に収まるようにしてください。"
        }
    } else {
        if name == "ポカ" {
